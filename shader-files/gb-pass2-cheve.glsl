...
uniform COMPAT_PRECISION float shadow_blur1;
#else
#define shadow_blur1 2.0
#endif
...
{
	#ifdef DEBUG
	vec4 out_c = COMPAT_TEXTURE(Source, clamp(vTexCoord, lower_bound, upper_bound));
	//out_c.rgb = out_c.rgb * out_c.a;
	//out_c.a = 1.0;
...
// I kept the old path when available for readability and speed(?)
	...
	float weights1 = 0.13; //0.13465834124289953661305802732548;   
    float weights2 = 0.13; //0.13051534237555914090930704141833;
    float weights3 = 0.13 - (shadow_blur1 / 100.0); //0.11883557904592230273554609080014;
    float weights4 = 0.13 - (3.0 * shadow_blur1 / 100.0); //0.10164546793794160274995705611009;
    float weights5 = 0.13 - (5.0 * shadow_blur1 / 100.0); //0.08167444001912718529866079800870;
    ...
    out_color.a += COMPAT_TEXTURE(Source, clamp(vTexCoord + vec2(offsets1 * texel.x, 0.0), lower_bound, upper_bound)).a * weights1;
    out_color.a += COMPAT_TEXTURE(Source, clamp(vTexCoord - vec2(offsets1 * texel.x, 0.0), lower_bound, upper_bound)).a * weights1;
	out_color.a += COMPAT_TEXTURE(Source, clamp(vTexCoord + vec2(offsets2 * texel.x, 0.0), lower_bound, upper_bound)).a * weights2;
    out_color.a += COMPAT_TEXTURE(Source, clamp(vTexCoord - vec2(offsets2 * texel.x, 0.0), lower_bound, upper_bound)).a * weights2;
	out_color.a += COMPAT_TEXTURE(Source, clamp(vTexCoord + vec2(offsets3 * texel.x, 0.0), lower_bound, upper_bound)).a * weights3;
    out_color.a += COMPAT_TEXTURE(Source, clamp(vTexCoord - vec2(offsets3 * texel.x, 0.0), lower_bound, upper_bound)).a * weights3;
	out_color.a += COMPAT_TEXTURE(Source, clamp(vTexCoord + vec2(offsets4 * texel.x, 0.0), lower_bound, upper_bound)).a * weights4;
    out_color.a += COMPAT_TEXTURE(Source, clamp(vTexCoord - vec2(offsets4 * texel.x, 0.0), lower_bound, upper_bound)).a * weights4;
	out_color.a += COMPAT_TEXTURE(Source, clamp(vTexCoord + vec2(offsets5 * texel.x, 0.0), lower_bound, upper_bound)).a * weights5;
    out_color.a += COMPAT_TEXTURE(Source, clamp(vTexCoord - vec2(offsets5 * texel.x, 0.0), lower_bound, upper_bound)).a * weights5;
#else
    ...
    */
    float weights[5] = float[]( 0.13, //0.13465834124289953661305802732548,       
                                ...
                                0.13 - (shadow_blur1 / 100.0), //0.11883557904592230273554609080014,         
                                0.13 - (3.0 * shadow_blur1 / 100.0), //0.10164546793794160274995705611009,      
                                0.13 - (5.0 * shadow_blur1 / 100.0)); //0.08167444001912718529866079800870 );      
    ...
    for (int i = 1; i < 5; i++)
    {
        out_color.a += COMPAT_TEXTURE(Source, clamp(vTexCoord + vec2(offsets[i] * texel.x, 0.0), lower_bound, upper_bound)).a * weights[i];
        out_color.a += COMPAT_TEXTURE(Source, clamp(vTexCoord - vec2(offsets[i] * texel.x, 0.0), lower_bound, upper_bound)).a * weights[i];
    }
#endif
